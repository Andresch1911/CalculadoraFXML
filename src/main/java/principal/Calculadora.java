...
public class Calculadora {
	...
	public StringProperty pantalla = new SimpleStringProperty();
	...
	public void insertarComa() {
		if (!pantalla.equals(""+COMA)){
			pantalla.set(pantalla.get().concat(""+COMA)); 
		}
	}
	...
	public void insertar(char digito) {
		if (digito >= '0' && digito <= '9') {
			...
			//pantalla += digito;
			pantalla.set(pantalla.get().concat(""+digito)); 
		} else if (digito == COMA) {
			insertarComa();
		}
	}
